...
class SprocketsTaglinkHandler extends icms_ipf_Handler {
    ...
    public function getTagsForObject($iid, &$handler, $label_type = '0') {
		...
		$clean_iid = isset($iid) ? (int)$iid : 0;
		$clean_label_type = isset($label_type) ? (int)$label_type : 0 ;
		...
		$sprockets_tag_handler = icms_getModuleHandler('tag', basename(dirname(__FILE__, 2)),
				'sprockets');
	...
	public function deleteTagsForObject(& $obj, $label_type)
	{
		...
		$sprockets_tag_handler = icms_getModuleHandler('tag', basename(dirname(__FILE__, 2)),
				'sprockets');
		$sprockets_taglink_handler = icms_getModuleHandler('taglink', basename(dirname(__FILE__, 2)),
				'sprockets');
